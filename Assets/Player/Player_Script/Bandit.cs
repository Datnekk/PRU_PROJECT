...
public class Bandit : NetworkBehaviour {
    ...
    void Attack()
    {
        ...
        Collider2D[] hitPlayers = Physics2D.OverlapCircleAll(attackPoint.position, m_attackRange, enemyLayerMask);
        foreach (Collider2D player in hitPlayers)
        {
            
            if (player.CompareTag("Player") && player.GetComponent<Warrior>())
            {
                player.GetComponent<Warrior>().TakeDamage(m_attackDamage);
